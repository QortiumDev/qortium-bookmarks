...
describe('Home settings host signals', () => {
  it('unwraps the Android Home settings bridge message', () => {
    const normalized = normalizeHomeSettingsHostMessage({
      type: 'qortium:home-settings-changed',
      detail: { theme: 'dark', accent: 'pink', textSize: 'large', ui: 'fun', language: 'fr' },
    });
    expect(getDisplaySettingsUpdateFromMessage(normalized, current)).toEqual({
...
describe('initial display settings', () => {
  ...
  it('reads uiStyle from the render URL before host globals', () => {
    vi.stubGlobal('window', {
      ...
      _qdnUiStyle: 'classic',
      location: {
        search: '?theme=dark&textSize=large&lang=fr&accent=blue&uiStyle=fun',
      },
    });
  ...
  it('continues to read the Modern style from the render URL', () => {
    vi.stubGlobal('window', {
      location: {
