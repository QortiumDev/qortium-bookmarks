...
const ENV_PREFIX = 'QORTIUM_BOOKMARKS';
const DEFAULT_NODE_API_URL = 'http://127.0.0.1:24891';
const DEFAULT_NAME = 'Bookmarks';
const DEFAULT_IDENTIFIER = 'Bookmarks';
const DEFAULT_TITLE = 'Bookmarks';
const DEFAULT_DESCRIPTION = 'Manage Qortium Home bookmarks, toolbar links, dashboard pins, and start pages.';
...
async function ensureNameRegistered(name, account) {
  ...
  const rawRegisterBytes58 = buildRegisterNameRawBytes58({
    ...
    data: JSON.stringify({
      app: DEFAULT_TITLE,
      purpose: 'QDN app preview for Bookmarks',
    }),
  });
...
// dist/ is gitignored, so publishing after a source update without rebuilding
// can otherwise ship an old bundle. Vite stamps the current package version
// into the JavaScript via __APP_VERSION__; require that stamp before loading
// keys or contacting the node.
...
// The helper signs through the node using a preview-account private key. Never
// send that key in cleartext to a remote host unless the operator explicitly
// accepts the risk for this invocation.
if (
  !isLoopbackNodeApiUrl() &&
  !nodeApiUrl.startsWith('https://') &&
  process.env.QORTIUM_BOOKMARKS_ALLOW_REMOTE_SIGN !== '1'
) {
  throw new Error(
    `Refusing to send the account private key to non-loopback HTTP node ${nodeApiUrl}. ` +
      'Use a local node or an https:// URL, or set QORTIUM_BOOKMARKS_ALLOW_REMOTE_SIGN=1 to override.',
  );
}
